...
function expandWord(cw) {
  ...
  // Enrichment fields
  if (cw.u) result.usage_notes = cw.u;
  if (cw.ex && cw.ex.length > 0) {
    result.extra_examples = cw.ex.map(x => ({ example_en: x.e, example_vi: x.v }));
  }
  if (cw.cp) result.common_patterns = cw.cp.split('|');
